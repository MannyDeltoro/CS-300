...
void BinarySearchTree::addNode(Node* node, Bid bid) {
    ...
    // adding node to the right of the tree
    else {
...
void BinarySearchTree::inOrder(Node* node) {
    ...
    if (node != nullptr) {
        inOrder(node->right);
        inOrder(node->left);
        cout << "bidid" << node->bid.bidId;
        cout << "title" << node->bid.title;
        cout << "amount" << node->bid.amount;
        cout << "fund" << node->bid.fund;
        // outputting the all the data from bid while node is null while using postorder in recursion 
...
void BinarySearchTree::preOrder(Node* node) {
    ...
    if (node != nullptr) {
        ...
        cout << "amount" << node->bid.amount;
        cout << "fund" << node->bid.fund;
        // outputting the all the data from bid while node is null while using preorder in recursion 
    }
      //if node is not equal to null ptr
      //output bidID, title, amount, fund
      //postOrder left
...
Node* BinarySearchTree::removeNode(Node* node, string bidId) {
    ...
    if (bidId.compare(node->bid.bidId) < 0) {
        ...
        // The node is no siblings (it's a leaf node).
        if (node->left == nullptr && node->right == nullptr) {
            delete node;
            node = nullptr;
        }
        // putting the nodes to the left
        else if (node->left != nullptr && node->right == nullptr) {
            Node* temp = node->left;
            node->bid = temp->bid;  // Swap the bid
            node->left = temp->left;  // Adjust links
            node->right = temp->right;
            delete temp;
        }
        // putting the nodes to the right 
        else if (node->right != nullptr && node->left == nullptr) {
            Node* temp = node->right;
            node->bid = temp->bid;  //swapping bids
            node->left = temp->left;  
            node->right = temp->right;
            delete temp;
        }
            ...
            node->bid = temp->bid;
            // repeatedly eliminate the left-most node.
